...
if ((GLOBAL1.countries.indexOf(country) !== -1)) { //if exists in data variable
  //console.log("one"+country+"|"+answer);
...
if (answer in count[country]){ //if you've seen an answer to the questino form this country
var num = count[country][answer]+1; //add to accumulated value
count[country][answer]=num;
var tot = count[country]["total"]+1;
count[country]["total"]=tot;
} else {
...
//the above is a list of lists so for every country,
//logging every question that appears as they key and the freq as the value for that question.
...
.on("click",function(d){svg.selectAll("*").remove(); drawCircles("Q27", d["answer"]);})
...
for (i=0; i<GLOBAL1.questions[activeQ][question].length; i++){
  ...
  total+=  count[country][value];
  }
  }
  for (i=0; i<GLOBAL1.questions[activeQ][question].length; i++){
    //var diff = count[country]["total"]-total;
    ////console.log(diff);
    var valvotes=0;
    var value= GLOBAL1.questions[activeQ][question][i]; //accessing the correct column
    if (count[country][value]!==undefined){
